...
import { Save, ShoppingCart, Eye, Edit2, X, Search, Trash2 } from 'lucide-react';
...
export default function OtherVendorForm() {
  ...
  useEffect(() => {
    const loadVendors = async () => {
      try {
        ...
        const vendorsResult = await syncService.load('otherVendors');
        ...
        const parsedVendors = allVendors.map(vendor => {
          ...
          ['address', 'primaryContact', 'bankDetails', 'paymentTerms'].forEach(field => {
  ...
  const handleEdit = (vendor) => {
    ...
    setFormData({
      ...
      vendorType: vendor.vendorType || 'Other Vendor',
      ...
      productServices: vendor.productServices || '',
  ...
  const handleDelete = async (vendorId) => {
    ...
    try {
      ...
      // Delete from server
      await databaseAPI.delete('otherVendors', vendorId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const filterVendorData = (vendorData) => {
        ...
        // Based on error, the table likely only has: id, code, status, createdAt, updatedAt, data
        ...
        // Explicitly iterate and filter
        for (const key in vendorData) {
          // Skip undefined
          if (vendorData[key] === undefined) continue;
          
          // Skip null values for optional fields (except id and code)
          if (vendorData[key] === null && !['id', 'code'].includes(key)) continue;
          
          // Only allow basic schema fields as direct columns
          if (allowedDirectColumns.includes(key)) {
            // This is an allowed direct column
        ...
        // Ensure we don't accidentally include any non-allowed fields
        const finalFiltered = {};
        allowedDirectColumns.forEach(col => {
          if (filtered[col] !== undefined) {
            finalFiltered[col] = filtered[col];
          }
        });
        
        // Always include data column if we have it
        if (filtered.data) {
          finalFiltered.data = filtered.data;
        }
      ...
      const vendorData = {
        ...formData,
        code: formData.vendorCode || `OV${String(vendors.length + 1).padStart(3, '0')}`,
      ...
      console.log('🔍 FILTERING CHECK - Original keys:', Object.keys(vendorData));
      console.log('✅ FILTERING CHECK - Filtered keys:', Object.keys(filteredVendorData));
      console.log('📦 FILTERING CHECK - Has companyName in filtered?', 'companyName' in filteredVendorData);
      console.log('📦 FILTERING CHECK - Data column:', filteredVendorData.data ? JSON.parse(filteredVendorData.data) : 'No data');
      
      // CRITICAL: Double-check that no non-allowed fields are present
      const allowedKeys = ['id', 'code', 'status', 'createdAt', 'updatedAt', 'data'];
      const unexpectedKeys = Object.keys(filteredVendorData).filter(k => !allowedKeys.includes(k));
      if (unexpectedKeys.length > 0) {
        console.error('❌ FILTERING ERROR - Unexpected keys found:', unexpectedKeys);
        console.error('❌ This should not happen! Filtering failed.');
        // Remove unexpected keys as a safety measure
        unexpectedKeys.forEach(key => delete filteredVendorData[key]);
      }

      // CRITICAL: Verify filtered data one more time before sending
      const finalData = { ...filteredVendorData };
      Object.keys(finalData).forEach(key => {
        if (!allowedKeys.includes(key)) {
          console.error(`🚨 CRITICAL ERROR: Removing unexpected key "${key}" from final data!`);
      ...
      if (editingId) {
        ...
        saveResult = await syncService.save('otherVendors', finalData, true, editingId);
        alert(`✅ Vendor "${formData.companyName}" updated successfully!`);
        // Dispatch event for vendor update
        window.dispatchEvent(new CustomEvent('vendorUpdated', { detail: { vendor: finalData } }));
      } else {
        // Create new vendor
        saveResult = await syncService.save('otherVendors', finalData);
        alert(`✅ Vendor "${formData.companyName}" created successfully!`);
        // Dispatch event for vendor creation
        window.dispatchEvent(new CustomEvent('vendorCreated', { detail: { vendor: finalData } }));
      }
      ...
      try {
        ...
        if (allVendors.length === 0 && saveResult?.fallback) {
          const localData = JSON.parse(localStorage.getItem('otherVendors') || '[]');
  ...
  const filteredVendors = vendors.filter(vendor => {
    ...
    return (
      ...
      (vendor.vendorCategory || '').toLowerCase().includes(searchLower) ||
                ...
                <div key={vendor.id} className="vendor-item">
                  <div className="vendor-info">
                    ...
                    <div style={{ fontSize: '0.9rem', color: '#64748b' }}>
                      ...
                      Category: {vendor.vendorCategory || 'N/A'} | 
          ...
          <div className="modal-overlay" onClick={() => setViewingId(null)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="form-section" style={{ marginBottom: '16px' }}>
                ...
                <div className="grid-2" style={{ marginTop: '16px' }}>
                  <div>
                    <label>Vendor Category</label>
                    <div style={{ padding: '8px', background: '#f8fafc', borderRadius: '6px' }}>
                      {viewingVendor.vendorCategory || 'N/A'}
                    </div>
                  </div>
                  <div>
                    <label>Status</label>
              ...
              <div className="form-section" style={{ marginBottom: '16px' }}>
                ...
                {viewingVendor.tdsApplicable && (
                  <div style={{ marginTop: '16px' }}>
                    <label>TDS Applicable</label>
                    <div style={{ padding: '8px', background: '#f8fafc', borderRadius: '6px' }}>
                      Yes {viewingVendor.tdsPercentage ? `(${viewingVendor.tdsPercentage}%)` : ''}
                    </div>
                  </div>
                )}
              </div>

              {viewingVendor.productServices && (
                <div className="form-section" style={{ marginBottom: '16px' }}>
                ...
                <div className="form-section" style={{ marginBottom: '16px' }}>
                  ...
                  <div className="grid-3">
                    ...
                    <div>
                      <label>Credit Limit</label>
                      <div style={{ padding: '8px', background: '#f8fafc', borderRadius: '6px' }}>
                        ₹{viewingVendor.paymentTerms.creditLimit || '0.00'}
                      </div>
                    </div>
                    <div>
                      <label>Payment Terms</label>
